...
import { updatePairDayData, updateTokenDayData, updateDefyswapDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = DefyswapFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  let defyswap = DefyswapFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  defyswap.totalLiquidityFTM = defyswap.totalLiquidityFTM.minus(pair.trackedReserveFTM as BigDecimal)
  ...
  if (bundle.ftmPrice.notEqual(ZERO_BD)) {
    trackedLiquidityFTM = getTrackedLiquidityUSD(bundle as Bundle, pair.reserve0, token0 as Token, pair.reserve1, token1 as Token).div(
      bundle.ftmPrice
    )
  } else {
  ...
  defyswap.totalLiquidityFTM = defyswap.totalLiquidityFTM.plus(trackedLiquidityFTM)
  defyswap.totalLiquidityUSD = defyswap.totalLiquidityFTM.times(bundle.ftmPrice)
...
export function handleMint(event: Mint): void {
  ...
  defyswap.txCount = defyswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let defyswap = DefyswapFactory.load(FACTORY_ADDRESS)
  defyswap.totalVolumeUSD = defyswap.totalVolumeUSD.plus(trackedAmountUSD)
  defyswap.totalVolumeFTM = defyswap.totalVolumeFTM.plus(trackedAmountFTM)
  defyswap.untrackedVolumeUSD = defyswap.untrackedVolumeUSD.plus(derivedAmountUSD)
  defyswap.txCount = defyswap.txCount.plus(ONE_BI)
  ...
  let defyswapDayData = updateDefyswapDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  defyswapDayData.dailyVolumeUSD = defyswapDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  defyswapDayData.dailyVolumeFTM = defyswapDayData.dailyVolumeFTM.plus(trackedAmountFTM)
  defyswapDayData.dailyVolumeUntracked = defyswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  defyswapDayData.save()
